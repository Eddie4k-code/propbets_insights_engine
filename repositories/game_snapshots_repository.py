...
class PostgresGameSnapshotsRepository(GameSnapshotsRepositoryInterface):
    ...
    def insert_game_snapshot(self, sport_key: str, season: int, game_id: int, game_ts: str, status: str, home_team_id: int, away_team_id: int, home_score: int, away_score: int):
        """
        ...
        with self.db.get_cursor() as cursor:
            cursor.execute(
                """
                INSERT INTO game_snapshots (sport_key, season, game_id, game_ts, status, home_team_id, away_team_id, home_score, away_score)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                ON CONFLICT (sport_key, season, game_id) 
                DO UPDATE SET 
                ...
                """,
                (sport_key, season, game_id, game_ts, status, home_team_id, away_team_id, home_score, away_score)
            )
